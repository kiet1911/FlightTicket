...
namespace AirlinesReservationSystem.Areas.Admin.Controllers
{
    ...
    public class FlightSchedulesController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "id,code,plane_id,from_airport,to_airport,departures_at,arrivals_at,totalSeats,cost")] FlightSchedule flightSchedule)
        {
            ...
            if(flightSchedule.departures_at.Date < datenow.Date)
            {
                ModelState.AddModelError("departures_at", "Chuyến bay không được nhỏ hơn ngày hiện tại.");
                viewbagitems();
                return View(flightSchedule);
            ...
            if (ModelState.IsValid)
            {
                ...
                else if (TotalHours >= 5 || TotalHours <= 0 || TotalHours < 0.5)// tránh việc nhập giờ bay cao 
                {
                    ModelState.AddModelError("arrivals_at", "Chuyến bay không được quá 5 tiếng  và ít nhất 30p.");
                    viewbagitems();
                    return View(flightSchedule);
                }
                ...
                {
                    AlertHelper.setAlert("false", "Tạo dữ liệu chuyến bay thất bại.");
                    viewbagitems();
                    return View(flightSchedule);
                }
            }
